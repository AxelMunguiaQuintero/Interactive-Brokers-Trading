...
def Open_Gap_Assets(trading_app: IB_Trading, suscripciones: list = [], menor_spread_porcentaje: bool = False) -> dict:
    ...
    trading_app : IB_Trading
        Instancia del objeto `IB_Trading` utilizado para realizar las peticiones al escáner y recuperar los datos.
    ...
    -------
    return: dict:
            Diccionario que contiene 2 DataFrames con los resultados del escáner:
            - Ganancia: Activos con mayor ganancia.
            - Perdida: Activos con mayor pérdida.
    """
    ...
    if len(suscripciones) == 0:
        ...
        suscripcion_top_gain.marketCapAbove = 500
        ...
        suscripcion_top_lose.marketCapAbove = 500
    ...
    if len(ids_peticiones) == 0:
        # No hay información actualmente almacenada
        id_peticion1 = 0
        id_peticion2 = 1
    else:
        # Usar IDs no utilizados anteriormente
    ...
    if menor_spread_porcentaje:
        ...
        for indice, registro_gain in peticion_gain.iterrows():
            ...
            # Validando que si se obtuvieron los datos
            if (precios_ask is not None) and (precios_mercado is not None) and (precios_bid is not None):
                ...
                # Rellenar Información con NaNs
                spreads_gain.append([np.nan] * 6)
...
if __name__ == "__main__":
    ...
    peticiones_escaner = Open_Gap_Assets(trading_app=IB_escaner, suscripciones=[], menor_spread_porcentaje=False)
    ...
    peticiones_escaner = Open_Gap_Assets(trading_app=IB_escaner, suscripciones=[], menor_spread_porcentaje=True)
