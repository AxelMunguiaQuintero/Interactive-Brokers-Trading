...
def Cruce_MA(df: pd.DataFrame, tendencia_rapida: int, tendencia_lenta: int, tendencia_continua: bool = True) -> pd.DataFrame:
    ...
    df : pd.DataFrame:
        ...
        Debe incluir una columna llamada 'Close' que representan los precios de cierre.
        
    tendencia_rapida : int
    ...
    tendencia_continua : bool, opcional
        Si es True, se mostrará la señal de cruce en todas las filas posteriores al cruce incial.
        Por defecto, es True.
    ...
    tendencia_corto_plazo = df["Close"].rolling(window=tendencia_rapida).mean()
...
if __name__ == "__main__":
    ...
    print(df)
    calculo_ma = Cruce_MA(df=df, tendencia_rapida=9, tendencia_lenta=21, tendencia_continua=True)    
    print(calculo_ma)
    ...
    ma_lines = [
        ...
        mpf.make_addplot(calculo_ma["MA_Rapida"].where(calculo_ma["Cruces"] == 1), color="green",
                         type="scatter", marker="X", markersize=150, alpha=1.0, panel=0),
    ...
    fig, axes = mpf.plot(df[["Open", "High", "Low", "Close", "Volume"]], style="yahoo", figsize=(22, 10),
                         title=dict(title="Promedio Móviles", size=20), volume=True, addplot=ma_lines, returnfig=True)
    ax_panel2 = axes[4]
